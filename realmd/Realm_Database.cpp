...
#define ADD_STMT(x, y) this->statement_holder[x] = this->connection->prepareStatement(y)
  ...
  RealmDatabaseConnection::open(const std::string& driver, const std::string& url)
  {
	...
	ADD_STMT(REALMD_DB_GET_REALMLIST, "SELECT id, name, address, port, icon, color, timezone, allowedSecurityLevel, population FROM realmlist WHERE color <>3 ORDER BY id");
